...
public:
	...
	const vector<BookReadingSession*>& GetReadingSessions() const{
         /*Note: Although the return is const vector, but the pointer is not const,
         so someone can cause problems using setters on pointer*/
...
public:
    ...
    void DisplaySession(BookReadingSession* session){
		...
		while(true){
            ...
            cout << session->GetCurrentContent() << "\n";
